...
:
    ...
    nbrPhiName_(dict.lookupOrDefault<word>("nbrPhi", "nbrPhi")),
...
void Foam::flowRateInletVelocityCoupledFvPatchVectorField::updateCoeffs()
{
    ...
    (
        nbrMesh
    ).boundary()[mpp.samplePolyPatch().index()];  // output is the mapped patch belonging to neighbor mesh
    ...
    //phi = phi*hocPyr_/qFuel;            // hocPyrosylate/ hocFuel
    ...
    scalarField U(-phi/patch().magSf());        // magsf is patch areas

    vectorField n(patch().nf());        // area normals
...
//        db().lookupObject<surfaceScalarField>(phiName_);
    ...
    if (phiName.dimensions() == dimVelocity*dimArea)
    {
        ...
        operator==(n*U);        // in each direction
    }
    else if (phiName.dimensions() == dimDensity*dimVelocity*dimArea)
    {
        ...
        if (debug)
        {
            scalar phi = gSum(rhop*(*this) & patch().Sf());     // Sf() returns face area vecs
...
//Info << "Dont know where the error occurs" << endl;
